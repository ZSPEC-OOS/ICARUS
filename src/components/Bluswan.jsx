...
export default function Bluswan({ onClose, models, setModels, selectedModelId, onModelChange, onSettingsChanged, onLogout, userEmail, savedModelIds, onModelSaved }) {
  ...
  const [lrmPhasePushed,   setLrmPhasePushed]   = useState(false)  // whether current phase was auto-pushed
  const [lrmPhasePrUrl,    setLrmPhasePrUrl]    = useState(null)   // PR URL for current phase
  ...
  const agentBranchRef = useRef(null)   // branch created for current agent run; cleared after PR
  ...
  const agentSession = useAgentSession({
    ...
    onAgentComplete: async (task, text) => {
      ...
      // Auto-create PR from the agent's working branch → baseBranch
  ...
  const handlePush = async () => {
    ...
    if (!dryRun && !confirmAction(
      `Push ${filesToPush.length} file${filesToPush.length !== 1 ? 's' : ''} to ${repoOwner}/${repoName} on a new branch, then open a PR`
    )) return
...
[{"id":1,"title":"Short title (5-8 words)","summary":"What this phase accomplishes in 1-2 sentences.","targets":["src/path/to/file.js"],"instructions":"Complete implementation instructions for this phase only."}]`
  ...
  const handleLrmStart = useCallback(() => {
    ...
    setLrmPhasePushed(false)
    setLrmPhasePrUrl(null)
  ...
  const handleLrmProceed = useCallback(async (fromIdx) => {
    ...
    // ── Step 1: Auto-push current phase to a branch + open PR ───────────────
    if (!lrmPhasePushed && hasGithub && !dryRun) {
      const phaseFiles = filePlan.filter(e => e.code?.trim())
      if (phaseFiles.length === 0) {
        // Nothing generated yet — tell user to wait for agent/generate to finish
        setLrmPlan(p => ({
          ...p,
          statuses: { ...p.statuses, [fromIdx]: 'blocked' },
          verifyError: `Phase ${fromIdx + 1} hasn't generated any files yet. Wait for it to complete.`,
        }))
        return
      }
      ...
      try {
        ...
        // Push each generated file to the phase branch
        for (const entry of phaseFiles) {
          ...
          await createOrUpdateFile(
            githubToken, repoOwner, repoName, entry.path, entry.code,
            `feat(phase-${fromIdx + 1}): ${phaseTitle}`,
            phaseBranch, existing?.sha || null
          )
        }

        // Open PR
        const prBody = [
          `## BLUSWAN LRM — Phase ${fromIdx + 1} of ${lrmPlan.phases.length}: ${phaseTitle}`,
          ``,
          `**Original request:** ${lrmPlan.originalPrompt}`,
          ``,
          `**Files changed (${phaseFiles.length}):**`,
          phaseFiles.map(e => `- \`${e.path}\``).join('\n'),
          ``,
          `---`,
          `*Merge this PR before proceeding to Phase ${fromIdx + 2}.*`,
        ].filter(Boolean).join('\n')
        const pr = await createPullRequest(
          githubToken, repoOwner, repoName,
          `BLUSWAN Phase ${fromIdx + 1}: ${phaseTitle}`,
          phaseBranch, baseBranch, prBody
        )

        setLrmPhasePushed(true)
        setLrmPhasePrUrl(pr?.html_url || null)
        setLastBranchName(phaseBranch)
        setPrResult({ url: pr?.html_url, number: pr?.number })
        logActivity('push', `✓ Phase ${fromIdx + 1} PR #${pr?.number} opened — merge on GitHub then click Proceed`)
        setLrmPlan(p => ({
          ...p,
          statuses: { ...p.statuses, [fromIdx]: 'blocked' },
          verifyError: `Phase ${fromIdx + 1} PR opened. Merge it on GitHub, then click Proceed to continue.`,
        }))
      } catch (err) {
    ...
    // ── Step 2: Already pushed — verify PR was merged via target files on baseBranch ──
      ...
      try {
        ...
        if (!anyFound) {
          setLrmPlan(p => ({
            ...p,
            statuses: { ...p.statuses, [fromIdx]: 'blocked' },
            verifyError: `Phase ${fromIdx + 1} PR not merged yet. Merge on GitHub, then click Proceed.`,
          }))
          return
        }
      } catch {
        // Network error — allow proceeding via override
      }
    }

    // ── Step 3: Merge verified — advance to next phase ───────────────────────
    setLrmPhasePushed(false)
    setLrmPhasePrUrl(null)
  ...
  }, [lrmPlan, lrmPhasePushed, filePlan, hasGithub, dryRun, githubToken, repoOwner, repoName, baseBranch, handleGenerate, logActivity])
    ...
    if (shouldUseAgent) {
      ...
      if (hasGithub && !dryRun) {
        try {
          ...
          agentBranchRef.current = newBranch
        } catch (err) {
          ...
          <div className="lk-input-card">
            ...
            <div className="lk-input-toolbar">
              ...
              <div className="lk-input-toolbar-right">
                ...
                {/* Push is now fully automatic — no manual button needed */}
