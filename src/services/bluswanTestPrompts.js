...
//
// Usage: import BLUSWAN_TEST_PROMPTS from './bluswanTestPrompts'
//        and pass them to the prompt input in BLUSWAN for manual or automated runs.
//
// Categories
...
export const BLUSWAN_TEST_PROMPTS = [
...

  {
    ...
    expectedBehaviour: 'BLUSWAN treats it as a normal coding request. No prompt injection succeeds. Generation produces a normal code response or gracefully declines.',
    triggersError: false,
    repairPathway: null,
  },
...

  {
    ...
    note: 'Navigate away from the BLUSWAN tab mid-generation.',
...

  {
    id: 'shd-05',
    category: 'shadowctx',
    label: 'BLUSWAN.md file is extremely large',
    prompt: 'Write a React component.',
    note: 'Create a BLUSWAN.md larger than BLUSWAN_MD_CAP (3000 chars). Verify truncation.',
    expectedBehaviour: 'BLUSWAN.md content is truncated to BLUSWAN_MD_CAP before injection. No context window explosion. Activity log may note "BLUSWAN.md truncated".',
    triggersError: false,
    repairPathway: 'bluswan-md-truncation',
  },
...

  {
    ...
    expectedBehaviour: 'Malformed event skipped gracefully. Console shows "[BLUSWAN] readSSEStream: skipped malformed event". Generation completes with all valid chunks. No crash.',
    triggersError: 'stream-malformed-event',
    repairPathway: 'stream-json-parse-catch',
  },
...
export const TEST_CATEGORIES = [...new Set(BLUSWAN_TEST_PROMPTS.map(p => p.category))]

// ── Filter helpers ─────────────────────────────────────────────────────────────
export const getTestsByCategory = (category) =>
  BLUSWAN_TEST_PROMPTS.filter(p => p.category === category)

export const getErrorTriggers = () =>
  BLUSWAN_TEST_PROMPTS.filter(p => p.triggersError !== false)

export const getRepairPathwayTests = () =>
  BLUSWAN_TEST_PROMPTS.filter(p => p.repairPathway)

// ── Summary stats ──────────────────────────────────────────────────────────────
export const getTestSummary = () => ({
  total:          BLUSWAN_TEST_PROMPTS.length,
...
export default BLUSWAN_TEST_PROMPTS
